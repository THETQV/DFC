...
void loop() {
  ...
  float azg = az / ACCEL_SCALE;
    ...
    float gyroRollRate = gx_e / 131.0f;
    float gyroPitchRate  = gy_e / 131.0f;
    float gyroYawRate   = gz_e / 131.0f;

    // Pitch and Roll calculation
    float eRoll = atan2(ayg, azg) * 180.0f / PI;
    float ePitch  = atan2(-axg, sqrt(ayg*ayg + azg*azg)) * 180.0f / PI;
  ...
  if (abs(roll) > 90) gyroPitchRate = -gyroPitchRate;
